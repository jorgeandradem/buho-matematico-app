 ...
 * NOTA INTERNA: MOTOR CERROJO MÁGICO v4.5 (MINI COFRE INTERACTIVO)
 */
...
const handleCheck = (val) => {
  ...
  if (val === currentChallenge.value.value) {
    ...
    // ACIERTO: Boost activo para el cofre abriéndose
    ...
    <main class="app-canvas adventure-bg">
      ...
      <div v-if="gameState === 'rules'" class="game-content flex-1 flex flex-col items-center justify-start p-6 pt-2 animate-fade-in gap-4">
        ...
        <div class="rules-panel shadow-2xl">
          ...
          <ul class="p-5 space-y-3 text-slate-700 font-bold text-[13px] leading-tight">
...
.chest-base-epic {
  ...
  width: 180px;
  height: 70px;
  background: linear-gradient(0deg, #78350f 0%, #92400e 100%);
  border: 6px solid #451a03;
  border-top: none;
...
.chest-mini-wrap.is-open .chest-lid-mini {
...
.rules-panel { width: 95%; max-width: 380px; background: white; border-radius: 2.5rem; border: 2px solid #e2e8f0; position: relative; }
