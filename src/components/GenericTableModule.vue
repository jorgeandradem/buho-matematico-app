  ...
  <div class="h-[100dvh] w-full bg-white flex justify-center overflow-hidden font-sans select-none text-slate-900">
    ...
    <div class="w-full max-w-[500px] h-full flex flex-col relative bg-white shadow-2xl overflow-hidden mx-auto border-x border-slate-100">
        ...
        <div v-if="!isNotebookMode" :class="`w-full h-full flex flex-col relative ${themeClasses.bg}`">
            ...
            <main class="flex-1 overflow-y-auto p-2 relative scrollbar-hide">
                <div class="grid grid-cols-2 gap-2 w-full">
                  <div v-for="ex in exercises" :key="ex.id" 
                       class="bg-white p-1 rounded-xl shadow-sm border-2 transition-all flex items-center justify-between gap-1 pl-3 pr-2 h-14"
                       :class="activeInputId === ex.id ? 'border-yellow-400 ring-2 ring-yellow-100 scale-[1.02] z-10' : 'border-white'">
                    <div class="text-xl font-black text-slate-600 font-numbers flex gap-0.5 whitespace-nowrap">
                    ...
                    <div class="relative flex items-center justify-end flex-1">
                      <div @click="focusInput(ex.id)"
                           class="w-16 h-10 rounded-lg border-2 flex items-center justify-center text-xl font-bold cursor-pointer transition-colors"
                      ...
                      <div v-if="userInputs[ex.id] === 'correct'" class="absolute right-0 mr-[-6px] text-green-600 animate-fade-in-scale z-20 bg-white rounded-full border border-green-100 shadow-sm"><Check :size="14" stroke-width="4" /></div>
                    </div>
                  </div>
                </div>
            </main>

            <footer class="flex-none bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-20">
                <VirtualKeypad @press="handleKeypadPress" @delete="handleDelete" />
            </footer>
        </div>

        <div v-if="isNotebookMode" class="absolute inset-0 z-50 bg-slate-100 flex flex-col overflow-hidden">
            
            <div v-if="selectedNotebookLevel === null" class="w-full h-full flex flex-col bg-white relative animate-fade-in">
                 <header class="flex-none p-6 pb-0">
                 ...
                 <main class="flex-1 flex flex-col items-center justify-center p-6 bg-slate-50/30">
                    <div class="bg-white rounded-[2.5rem] shadow-xl border-4 border-white w-full max-w-sm p-8 flex flex-col items-center relative overflow-hidden ring-1 ring-slate-100">
                        ...
                        <div class="relative flex flex-col items-center mb-8 text-center">
                            ...
                            <h1 :class="`text-3xl font-black uppercase tracking-wider ${themeClasses.text}`">{{ title }}</h1>
                            <p class="text-slate-400 font-bold text-xs uppercase tracking-widest mt-1">Selecciona tu desafío</p>
                        </div>
                        <div class="grid grid-cols-1 gap-4 w-full relative z-10">
                            <button @click="selectedNotebookLevel = 1" class="group bg-slate-50 hover:bg-white p-4 rounded-2xl border-2 border-slate-100 hover:border-green-400 transition-all flex items-center gap-4 shadow-sm active:scale-95 border-b-4">
                                <div class="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center text-green-600 group-hover:scale-110 transition-transform"><BookOpen :size="24" stroke-width="3" /></div>
                                <div class="text-left">
                                    <h3 class="text-xl font-black text-slate-700">Nivel 1</h3>
                                    <p class="text-slate-400 font-bold text-[10px] uppercase">Tablas del 1 al 10</p>
                                </div>
                            </button>
                            <button @click="selectedNotebookLevel = 2" class="group bg-slate-50 hover:bg-white p-4 rounded-2xl border-2 border-slate-100 hover:border-purple-400 transition-all flex items-center gap-4 shadow-sm active:scale-95 border-b-4">
                                <div class="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center text-purple-600 group-hover:scale-110 transition-transform"><Calculator :size="24" stroke-width="3" /></div>
                                <div class="text-left">
                                    <h3 class="text-xl font-black text-slate-700">Nivel 2</h3>
                                    <p class="text-slate-400 font-bold text-[10px] uppercase">2 y 3 cifras</p>
                                </div>
                            </button>
                        </div>
                    </div>
