...
import { 
    X as CloseIcon, Trophy, Zap, AlertCircle, Flag, Ship, 
    MousePointer2, PlayCircle, ChevronRight, Volume2 
...
const vocalizeRules = () => {
    speak("¡Bienvenido Capitán a la Ruta del Tesoro! Navega a través de diez islas resolviendo operaciones matemáticas. Cada acierto abre el cofre y te permite zarpar. ¡Evita los errores para no perder tu botín!");
};
...
const finishGame = async () => {
    ...
    if (totalErrors.value > 6) {
        sessionCoins.value = { gold: 0, silver: 0, copper: 5 };
    }

    // Vocalización de Salida: Narra premio y botín
    const resultText = `¡Ruta completada Capitán! Has ganado un botín de ${sessionCoins.value.gold} monedas de oro, ${sessionCoins.value.silver} de plata y ${sessionCoins.value.copper} de cobre.`;
...
.map-3d-epic { position: relative; width: 110px; height: 110px; margin-bottom: 1.5rem; transform: rotateX(40deg) rotateZ(-15deg) rotateY(15deg); transform-style: preserve-3d; }
.map-surface { position: absolute; inset: 0; background: radial-gradient(circle at center, #fef08a 0%, #facc15 100%); border: 4px solid #92400e; border-radius: 6px; box-shadow: -1px 1px 0px #78350f, -2px 2px 0px #78350f, -3px 3px 0px #78350f, -4px 4px 0px #78350f, -5px 5px 0px #78350f, -15px 15px 25px rgba(0,0,0,0.4); overflow: hidden; z-index: 5; }
.map-scroll-top { position: absolute; top: -12px; left: -10px; right: -10px; height: 24px; background: linear-gradient(to bottom, #f59e0b, #b45309); border: 3px solid #78350f; border-radius: 12px; box-shadow: -4px 4px 0 #451a03; z-index: 10; }
.map-scroll-bottom { position: absolute; bottom: -12px; left: -10px; right: -10px; height: 24px; background: linear-gradient(to top, #f59e0b, #b45309); border: 3px solid #78350f; border-radius: 12px; box-shadow: -4px 4px 0 #451a03; z-index: 10; }
@keyframes mapDash { to { stroke-dashoffset: -20; } }
.animate-map-dash { stroke-dashoffset: 0; animation: mapDash 1.5s linear infinite; }
